  ...
  return (
    <div className='bg-red-gradient max-w-md w-full mx-auto my-60 py-4 rounded-md flex flex-col justify-center items-center gap-4'>
      <form onSubmit={handleSubmit}
        ...
        <h1 className='font-main text-yellow my-4 '>{t('login.login')}</h1>
        <input name="email" onChange={handleChange} value={form.email}
          className='p-2 outline-none w-2/3 rounded-lg ' type="email" placeholder={t('login.emailPlaceHolder')} />
        <div className='w-2/3 '>
        ...
        <input name="password" onChange={handleChange} value={form.password}
          className='p-2 outline-none w-2/3 rounded-lg' type="password" placeholder={t('login.passwordPlaceHolder')} />
        <div className='w-2/3 '>
          {formatError && <p className='text-gray-300 text-xs'>{formatError.password}</p>}
        </div>
        <button className='bg-yellow text-white py-4 px-6 font-head  rounded-xl'>{t('login.login')} </button>
      </form>
      <p onClick={() => setIsOpen(!isOpen)} className='text-white cursor-pointer underline hover:text-yellow'>{t('login.forgetPassword')}</p>
